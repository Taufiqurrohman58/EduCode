...
class _Level8ScreenState extends State<Level8Screen>
  ...
  void handlePanEnd() {
    ...
    LineSegment userLine = LineSegment(drawnPoints.first!, drawnPoints.last!);

    for (var line in missingLines[activeCell]) {
      if (completedLines[activeCell]!.contains(line)) continue;

      if (userLine.isCloseTo(line)) {
        setState(() {
          completedLines[activeCell]!.add(line);
        });
  ...
  void _nextLevel8Screen() async {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Level8Screen 9 terbuka')),
    );
